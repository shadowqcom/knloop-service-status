async function updateChart() {
    try {
        ...
        const oneDayAgo = new Date(startOfCurrentHour.getTime() - (24 * 60 * 60 * 1000));

        const response = await fetch('./logs/Mojoo_report.log');
        if (!response.ok) {
        ...
        // 创建一个完整的时间序列，从当前时间的前23小时到当前时间的整点
        const labels = [];
        const data = [];
        ...
        const chart = new Chart(ctx, {
            ...
            data: {
                labels,
                datasets: [{
                    // label: 'Average Delay per Hour (ms)',
                    data,
                    fill: false,
                    borderColor: '#4bc0c0',
                    tension: 0.4
...
// 每分钟更新一次图表
// setInterval(updateChart, 6 * 1000);